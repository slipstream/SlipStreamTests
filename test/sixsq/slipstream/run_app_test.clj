...
(ns sixsq.slipstream.run-app-test
  (:require
    ...
    [sixsq.slipstream.test-base :refer [fixture-terminate get-config
                                        http-quiet!
                                        inst-names-range is-url
                                        is-uuid run-uuid-from-run-url set-run-uuid
...
(deftest test-deploy-terminate
  ...
  (let [{:keys [username password endpoint comp-name
                app-uri connector-name insecure?]} (get-config)
        deploy-params-map (-> {}
